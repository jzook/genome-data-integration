...
while ($line = <IN>) {
    ...
    @fields = split( ",", $line);
     if ($res == 0) {
	 my $samCmd = "java -jar -Xmx2g  ~/bioinfo/GenomeAnalysisTK-2.1-8-g5efb575/GenomeAnalysisTK.jar -R /data/results/justin/GRCh37/human_g1k_v37.fasta -T CallableLoci -I $fields[0] -summary $fields[0].callable.summary -o $fields[0].frlmq01.mlmq10.mbq10.mdflmq5.dp5.mmq10.callable.bed -frlmq 0.1 -mlmq 10 -mbq 10 -minDepth 5 --minDepthForLowMAPQ 5 -mmq 10 &";
	  print "$samCmd\n";
	 $res = system($samCmd);
    }   
