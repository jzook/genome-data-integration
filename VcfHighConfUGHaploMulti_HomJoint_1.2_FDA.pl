...
#                if ($chromposbest[$j]==1016359050) {
			...
			$j++;
                ...
                if (($chrombest[$maxdataset]==0)) {
                    ...
                    print "$chrompos[$maxdataset]:@chromposbest\n";
                    print "$refmax:@refbest; altvarlong:$altVarLong\n";
					print  "$chrombest[$maxdataset]\t$posbest[$maxdataset]\t$idbest[$maxdataset]\t$refmax\t$altVarLong\t$PLmin\tPASS\t$infobest[$maxdataset];geno=$geno;PLtot=$PLmin;NoPLnum=$NoPL;DPTot=$DPTot;PLbyDP=$PLbyDP;HapNoVar=$HapNoVar;LowMQ0Fraction=$MQ0;maxdataset=$infiles[$maxdataset]$allPLannot\tGT:DP:PL\t$GTs[$geno-1]:$DPTot:$PLTottxt\n\n";
                }
