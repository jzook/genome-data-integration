...
if ($#ARGV < 5) {
    print "$#ARGV arguments\n usage: VcfClassifyUsingVQSR_snpindel_v2.15.pl inputvcfUGHapMergefilenamestart chromosomeName datasetNames(>=2 separated by spaces and alternating with platform identifier)\n";
    print "example: perl /Applications/bioinfo/perl/VcfClassifyUsingVQSR_snpindel_v2.15.pl AllFDAdatasets_130517_ all HSWG ill ILLWG ill XIll ill 454WG 454 ILLCLIA ill IllPCRFree ill XPSolWGLS sol IonEx ion HSWEx ill ILLWEx ill CG cg PlatGen ill\n";
 exit;
}
...
unless ( open(OUTPUT, ">${infilestart}allcall_UGHapMerge_VQSRv2.15_${chrom}.vcf")) {
    print "\nCannot open the file: ${infilestart}allcall_UGHapMerge_VQSRv2.15_${chrom}.vcf to write to! \n\n";
    exit;
}
unless ( open(OUTHOMREF, ">${infilestart}allcall_UGHapMerge_HomRef_VQSRv2.15_${chrom}.vcf")) {
    print "\nCannot open the file: ${infilestart}allcall_UGHapMerge_HomRef_VQSRv2.15_${chrom}.vcf to write to! \n\n";
    exit;
}
unless ( open(OUTHET, ">${infilestart}allcall_UGHapMerge_Het_VQSRv2.15_${chrom}.vcf")) {
    print "\nCannot open the file: ${infilestart}allcall_UGHapMerge_Het_VQSRv2.15_${chrom}.vcf to write to! \n\n";
    exit;
}
unless ( open(OUTHOMVAR, ">${infilestart}allcall_UGHapMerge_HomVar_VQSRv2.15_${chrom}.vcf")) {
    print "\nCannot open the file: ${infilestart}allcall_UGHapMerge_HomVar_VQSRv2.15_${chrom}.vcf to write to! \n\n";
    exit;
}
unless ( open(OUTVARPASS, ">${infilestart}allcall_UGHapMerge_HetHomVarPASS_VQSRv2.15_${chrom}.vcf")) {
    print "\nCannot open the file: ${infilestart}allcall_UGHapMerge_HetHomVarPASS_VQSRv2.15_${chrom}.vcf to write to! \n\n";
    exit;
}
unless ( open(OUTVARALL, ">${infilestart}allcall_UGHapMerge_HetHomVarAll_VQSRv2.15_${chrom}.vcf")) {
    print "\nCannot open the file: ${infilestart}allcall_UGHapMerge_HetHomVarAll_VQSRv2.15_${chrom}.vcf to write to! \n\n";
    exit;
}
...
foreach my $fh (@infilehandlesDistHomAB) {
    ...
    while ($line = <$fh>) {
		...
		if ($fhn==0) {print OUTVARPASS $line;}
		if ($fhn==0) {print OUTVARALL $line;}
        if ($x==1) {last;}
    }
    $fhn++;
...
while($endlineall==1) {
    ...
    foreach my $fh (@infilehandlesDistHetAlign) {
		for (my $j=0;$j<$readnewline[$i];$j++) {
            ...
            my $filter=$fields[6];
            $VQSRHetAlign[$i]=0;
            if ($fields[7] =~ /ReadPosEndDist/) { #only use VQSRAlign if ReadPosEndDist has a value            if ($filter =~ /99.90to/) { $VQSRHetAlign[$i]=99.9;
    ...
    for (my $i=0;$i<=$#infilehandlesDistHomAB;$i++) {
		if ($chromposHom[$i]!=$minchrompos && $chromposHet[$i]!=$minchrompos) {
            if ($chromposHet[$i]<=$minchrompos+20) {
                if (!($qualHet[$i] eq ".") && $qualHet[$i]>40 && ($infoHet[$i] =~ /UGHap=Hap/ || $infoHet[$i] =~ /UGHap=Both/)) {
                    if (!($PLtextHet[$i] eq "")) {
                        my @PLs = split( ",", $PLtextHet[$i]);
                        $PLno=$#PLs;
                        my $j=0; my $PL0=0; my $PLmin=1000000;
            ...
            } elsif ($chromposHetprev[$i]>=$minchrompos-20) {
                if (!($qualHetprev[$i] eq ".") && $qualHetprev[$i]>40 && ($infoHetprev[$i] =~ /UGHap=Hap/ || $infoHetprev[$i] =~ /UGHap=Both/)) {
            ...
            if ($chromposHom[$i]==$minchrompos && ($genoHom[$i]==1 || $genoHom[$i]==3 || $genoHom[$i]==6 || $genoHom[$i]==10 || $genoHom[$i]==15)) {
                ...
                #if this dataset has a haplotypecaller variant call within 20 bases but no variant at this position, then something went wrong
                if ($infoHom[$i] =~ /UGHap=HapRefUGVar/) {
    ...
    for (my $kk=0; $kk<$datasetcalls; $kk++) {
        ...
        if ($kk>0 && !($altVar[$kk] eq $altVarLong)) { $multiplealts=1;}
        if ($j>$altno) {$altno=$j; $altVarLong=$altVar[$kk]; $refVarLong=$refVar[$kk];}
    }
    ...
    for (my $kk=0; $kk<$datasetcalls; $kk++) {
        ...
        if ($PLmin>20) {
            ...
            if ($platformgenotxt[$genoVar[$kk]-1] =~ /,$platforms[$kk],/) {} else {
                $platformgenotxt[$genoVar[$kk]-1] = "$platformgenotxt[$genoVar[$kk]-1]$platforms[$kk],";
            }
    ...
    if ($DPSum>0 && (($varType eq "SNP" && $PLminsum>$MinPLSumSNP[$genosum-1] && $PLbyDPsum>$MinPLbyDPSumSNP[$genosum-1] && $YesPLTot[$genosum-1]>1 && ($NoPLTot[$genosum-1]==0 || $YesPLTot[$genosum-1]/($NoPLTot[$genosum-1]+0.0)>=5) && ($HapNoVar<2 || $genosum==1)) || \
        !($varType eq "SNP") && $PLminsum>$MinPLSumINDEL[$genosum-1] && $PLbyDPsum>$MinPLbyDPSumINDEL[$genosum-1] && $YesPLTot[$genosum-1]>1 && ($NoPLTot[$genosum-1]==0 || $YesPLTot[$genosum-1]/($NoPLTot[$genosum-1]+0.0)>=5) && ($HapNoVar<2 || $genosum==1))) {
        $filtergeno="PASS";
    ...
    if ($filtergeno eq "PASS") {
        ...
        if ((($genosum==3 || $genosum==6 || $genosum==10 || $genosum==15) && $TrancheABQDmin2>=99) || (($genosum!=1 && $genosum!=3 && $genosum!=6 && $genosum!=10 && $genosum!=15) && $TrancheABQDmin2>=95)) {
    ...
    } elsif ($datasetcalls<2 || $datasetcalls<$MAPGOODMIN) {
        $filtergeno="filter${datasetcalls}Dataset(s)"
    } elsif ($DPSum<20) {
        $filtergeno="filterCov$DPSum";
    } elsif ($HapNoVar>=2) {
        $filtergeno="filterHapNoVar";
    } else {
        $filtergeno="filterConflictingGenotypes";
    }
    if ($chromVar[0] eq "0") { next; } #skip output if no datasets exist at this row
    my $PL0diffsum=$PLTot[0]-$PLTot[$genosum-1];

    $PLbyDPsum=int($PLbyDPsum*100+0.5)/100.0;
            print OUTPUT "$chromVar[0]\t$posVar[0]\t$idVar[0]\t$refVarLong\t$altVarLong\t$PL0diffsum\t$filtergeno\tgeno=$genosum;platforms=$platformcount;platformnames=$platformtxt;varType=$varType;YesPLtot=$YesPLTot[$genosum-1];NoPLTot=$NoPLTot[$genosum-1];HapNoVar=$HapNoVar;PLminsum=$PLminsum;DPSum=$DPSum;PLminsumOverDP=$PLbyDPsum;genoMapGood=$mapgood[$genosum-1];TrancheSSEmin2=$TrancheSSEmin2;TrancheABQDmin2=$TrancheABQDmin2;TrancheAlignmin2=$TrancheAlignmin2;TrancheMapmin2=$TrancheMapmin2;HapNoVar=$HapNoVar;datasetcalls=$datasetcalls$allPLannot\tGT:DP:GQ:PL\t$GTs[$genosum-1]:$DPSum:$PLminsum:$PLTottxt\n";
            if ($genosum==1) {
