...
vqsr_1000G_phase1_indel="/home/regression/references/hg19/1000G_phase1.indels.hg19.sites.vcf.gz"
...
workdir="$HOME/testcase" #Determine where the output files will be stored
...
$release_dir/bin/bwa mem -M -R "@RG\tID:$group\tSM:$sample\tPL:$platform" -t $nt $fasta $fastq_folder/$fastq_1 $fastq_folder/$fastq_2 | $release_dir/bin/sentieon util sort -o sorted.bam -t $nt --sam2bam -i -
...
if [ "$run_vqsr" = "yes" ]; then
	...
	#Run the VQSR
	$release_dir/bin/sentieon driver -r $fasta --algo VarCal -v output-hc.vcf.gz $resource_text $annotate_text --var_type SNP --plot_file vqsr_SNP.hc.plot_file.txt --nthr 1 --max_gaussian 8 --srand 47382911 --tranches_file vqsr_SNP.hc.tranches vqsr_SNP.hc.recal
	#apply the VQSR
	...
	#create the resource argument
	resource_text="--resource $dbsnp_1000G_phase1_indel --resource_param 1000G,known=false,training=true,truth=false,prior=10.0 "
	resource_text="$resource_text --resource $dbsnp_Mill --resource_param Mills,known=false,training=true,truth=true,prior=12.0 "
	resource_text="$resource_text --resource $dbsnp --resource_param dbsnp,known=true,training=false,truth=false,prior=2.0 "
	#create the annotation argument
	...
	#Run the VQSR
	$release_dir/bin/sentieon driver -r $fasta --algo VarCal -v vqsr_SNP.hc.recaled.vcf.gz $resource_text $annotate_text --var_type INDEL --plot_file vqsr_SNP_INDEL.hc.plot_file.txt --nthr 1 --max_gaussian 4 --srand 47382911 --tranches_file vqsr_SNP_INDEL.hc.tranches vqsr_SNP_INDEL.hc.recal
	#apply the VQSR
