...
workdir="$HOME/testcaseMultisample" #Determine where the output files will be stored
...
for sample in sample1 sample2 sample3; do
  ...
  # ******************************************
  $release_dir/bin/bwa mem -M -R "@RG\tID:$group\tSM:$sample\tPL:$platform" -t $nt $fasta $fastq_folder/${sample}_$fastq_1_suffix $fastq_folder/${sample}_$fastq_2_suffix | $release_dir/bin/sentieon util sort -o sorted.bam -t $nt --sam2bam -i -
