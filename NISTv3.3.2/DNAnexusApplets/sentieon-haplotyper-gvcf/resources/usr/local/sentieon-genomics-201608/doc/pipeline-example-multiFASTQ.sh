...
# using a single sample with more that one
...
workdir="$HOME/testcaseMultifile" #Determine where the output files will be stored
...
for i in $(seq 1 $num_sets); do
 ...
 $release_dir/bin/bwa mem -M -R "@RG\tID:${group_prefix}_$i\tSM:$sample\tPL:$platform" -t $nt $fasta $fastq_folder/$fastq_prefix$i$fastq_suffix_1 $fastq_folder/$fastq_prefix$i$fastq_suffix_2 | $release_dir/bin/sentieon util sort -o sorted_set$i.bam -t $nt --sam2bam -i -
done

# ******************************************
# 2. Metrics on the multiple sorted BAM files
# ******************************************
$release_dir/bin/sentieon driver -r $fasta -t $nt $bam_input --algo MeanQualityByCycle mq_metrics.txt --algo QualDistribution qd_metrics.txt --algo GCBias --summary gc_summary.txt gc_metrics.txt --algo AlignmentStat aln_metrics.txt --algo InsertSizeMetricAlgo is_metrics.txt
