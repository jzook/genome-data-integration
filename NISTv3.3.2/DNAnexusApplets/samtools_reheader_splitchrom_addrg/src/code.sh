...
#    Note that chrM is not changed to MT because sometimes chrM implies a different ref from MT and causes collisions downstream
...
samtools view -buh input.bam chr1 | samtools reheader input_v37.sam - | $java -jar /AddOrReplaceReadGroups.jar I=/dev/stdin O=bam1.bam RGID=$rgid RGLB=$rglb RGPL=$rgpl RGPU=$rgpu RGSM=$rgsm $opts
samtools index bam1.bam bam1.bai &
samtools view -buh input.bam chr2 | samtools reheader input_v37.sam - | $java -jar /AddOrReplaceReadGroups.jar I=/dev/stdin O=bam2.bam RGID=$rgid RGLB=$rglb RGPL=$rgpl RGPU=$rgpu RGSM=$rgsm $opts
samtools index bam2.bam bam2.bai &
samtools view -buh input.bam chr3 | samtools reheader input_v37.sam - | $java -jar /AddOrReplaceReadGroups.jar I=/dev/stdin O=bam3.bam RGID=$rgid RGLB=$rglb RGPL=$rgpl RGPU=$rgpu RGSM=$rgsm $opts
samtools index bam3.bam bam3.bai &
samtools view -buh input.bam chr4 | samtools reheader input_v37.sam - | $java -jar /AddOrReplaceReadGroups.jar I=/dev/stdin O=bam4.bam RGID=$rgid RGLB=$rglb RGPL=$rgpl RGPU=$rgpu RGSM=$rgsm $opts
samtools index bam4.bam bam4.bai &
samtools view -buh input.bam chr5 | samtools reheader input_v37.sam - | $java -jar /AddOrReplaceReadGroups.jar I=/dev/stdin O=bam5.bam RGID=$rgid RGLB=$rglb RGPL=$rgpl RGPU=$rgpu RGSM=$rgsm $opts
samtools index bam5.bam bam5.bai &
samtools view -buh input.bam chr6 | samtools reheader input_v37.sam - | $java -jar /AddOrReplaceReadGroups.jar I=/dev/stdin O=bam6.bam RGID=$rgid RGLB=$rglb RGPL=$rgpl RGPU=$rgpu RGSM=$rgsm $opts
samtools index bam6.bam bam6.bai &
samtools view -buh input.bam chr7 | samtools reheader input_v37.sam - | $java -jar /AddOrReplaceReadGroups.jar I=/dev/stdin O=bam7.bam RGID=$rgid RGLB=$rglb RGPL=$rgpl RGPU=$rgpu RGSM=$rgsm $opts
samtools index bam7.bam bam7.bai &
samtools view -buh input.bam chr8 | samtools reheader input_v37.sam - | $java -jar /AddOrReplaceReadGroups.jar I=/dev/stdin O=bam8.bam RGID=$rgid RGLB=$rglb RGPL=$rgpl RGPU=$rgpu RGSM=$rgsm $opts
samtools index bam8.bam bam8.bai &
samtools view -buh input.bam chr9 | samtools reheader input_v37.sam - | $java -jar /AddOrReplaceReadGroups.jar I=/dev/stdin O=bam9.bam RGID=$rgid RGLB=$rglb RGPL=$rgpl RGPU=$rgpu RGSM=$rgsm $opts
samtools index bam9.bam bam9.bai &
samtools view -buh input.bam chr10 | samtools reheader input_v37.sam - | $java -jar /AddOrReplaceReadGroups.jar I=/dev/stdin O=bam10.bam RGID=$rgid RGLB=$rglb RGPL=$rgpl RGPU=$rgpu RGSM=$rgsm $opts
samtools index bam10.bam bam10.bai &
samtools view -buh input.bam chr11 | samtools reheader input_v37.sam - | $java -jar /AddOrReplaceReadGroups.jar I=/dev/stdin O=bam11.bam RGID=$rgid RGLB=$rglb RGPL=$rgpl RGPU=$rgpu RGSM=$rgsm $opts
samtools index bam11.bam bam11.bai &
samtools view -buh input.bam chr12 | samtools reheader input_v37.sam - | $java -jar /AddOrReplaceReadGroups.jar I=/dev/stdin O=bam12.bam RGID=$rgid RGLB=$rglb RGPL=$rgpl RGPU=$rgpu RGSM=$rgsm $opts
samtools index bam12.bam bam12.bai &
samtools view -buh input.bam chr13 | samtools reheader input_v37.sam - | $java -jar /AddOrReplaceReadGroups.jar I=/dev/stdin O=bam13.bam RGID=$rgid RGLB=$rglb RGPL=$rgpl RGPU=$rgpu RGSM=$rgsm $opts
samtools index bam13.bam bam13.bai &
samtools view -buh input.bam chr14 | samtools reheader input_v37.sam - | $java -jar /AddOrReplaceReadGroups.jar I=/dev/stdin O=bam14.bam RGID=$rgid RGLB=$rglb RGPL=$rgpl RGPU=$rgpu RGSM=$rgsm $opts
samtools index bam14.bam bam14.bai &
samtools view -buh input.bam chr15 | samtools reheader input_v37.sam - | $java -jar /AddOrReplaceReadGroups.jar I=/dev/stdin O=bam15.bam RGID=$rgid RGLB=$rglb RGPL=$rgpl RGPU=$rgpu RGSM=$rgsm $opts
samtools index bam15.bam bam15.bai &
samtools view -buh input.bam chr16 | samtools reheader input_v37.sam - | $java -jar /AddOrReplaceReadGroups.jar I=/dev/stdin O=bam16.bam RGID=$rgid RGLB=$rglb RGPL=$rgpl RGPU=$rgpu RGSM=$rgsm $opts
samtools index bam16.bam bam16.bai &
samtools view -buh input.bam chr17 | samtools reheader input_v37.sam - | $java -jar /AddOrReplaceReadGroups.jar I=/dev/stdin O=bam17.bam RGID=$rgid RGLB=$rglb RGPL=$rgpl RGPU=$rgpu RGSM=$rgsm $opts
samtools index bam17.bam bam17.bai &
samtools view -buh input.bam chr18 | samtools reheader input_v37.sam - | $java -jar /AddOrReplaceReadGroups.jar I=/dev/stdin O=bam18.bam RGID=$rgid RGLB=$rglb RGPL=$rgpl RGPU=$rgpu RGSM=$rgsm $opts
samtools index bam18.bam bam18.bai &
samtools view -buh input.bam chr19 | samtools reheader input_v37.sam - | $java -jar /AddOrReplaceReadGroups.jar I=/dev/stdin O=bam19.bam RGID=$rgid RGLB=$rglb RGPL=$rgpl RGPU=$rgpu RGSM=$rgsm $opts
samtools index bam19.bam bam19.bai &
samtools view -buh input.bam chr20 | samtools reheader input_v37.sam - | $java -jar /AddOrReplaceReadGroups.jar I=/dev/stdin O=bam20.bam RGID=$rgid RGLB=$rglb RGPL=$rgpl RGPU=$rgpu RGSM=$rgsm $opts
samtools index bam20.bam bam20.bai &
samtools view -buh input.bam chr21 | samtools reheader input_v37.sam - | $java -jar /AddOrReplaceReadGroups.jar I=/dev/stdin O=bam21.bam RGID=$rgid RGLB=$rglb RGPL=$rgpl RGPU=$rgpu RGSM=$rgsm $opts
samtools index bam21.bam bam21.bai &
samtools view -buh input.bam chr22 | samtools reheader input_v37.sam - | $java -jar /AddOrReplaceReadGroups.jar I=/dev/stdin O=bam22.bam RGID=$rgid RGLB=$rglb RGPL=$rgpl RGPU=$rgpu RGSM=$rgsm $opts
samtools index bam22.bam bam22.bai &
samtools view -buh input.bam chrX | samtools reheader input_v37.sam - | $java -jar /AddOrReplaceReadGroups.jar I=/dev/stdin O=bamX.bam RGID=$rgid RGLB=$rglb RGPL=$rgpl RGPU=$rgpu RGSM=$rgsm $opts
samtools index bamX.bam bamX.bai &
samtools view -buh input.bam chrY | samtools reheader input_v37.sam - | $java -jar /AddOrReplaceReadGroups.jar I=/dev/stdin O=bamY.bam RGID=$rgid RGLB=$rglb RGPL=$rgpl RGPU=$rgpu RGSM=$rgsm $opts
samtools index bamY.bam bamY.bai 
samtools view -buh input.bam chrM | samtools reheader input_v37.sam - | $java -jar /AddOrReplaceReadGroups.jar I=/dev/stdin O=bamMT.bam RGID=$rgid RGLB=$rglb RGPL=$rgpl RGPU=$rgpu RGSM=$rgsm $opts
