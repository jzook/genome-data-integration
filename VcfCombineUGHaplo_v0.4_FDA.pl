...
# If HaplotypeCaller and UnifiedGenotyper agree at a site for a dataset, then use UnifiedGenotyper info for integration. If HaplotypeCaller and UnifiedGenotyper disagree at a site for a dataset, then: (1) for SNPs, if no nearby (<=35bp) indels/SNPs differ between HaplotypeCaller and UnifiedGenotyper, then use the UnifiedGenotyper call, or (2) for SNPs, if nearby (<=35bp) indels/SNPs differ between HaplotypeCaller and UnifiedGenotyper, then use the HaplotypeCaller call, or (3) for indels and MNPs, use the HaplotypeCaller call (or filter UnifiedGenotyper call)
...
#				8/29/13 - now check for 2 previous haplotype calls since it kept some UG calls after a complex indel-snp event
# Version 0.4 - Oct 29, 2013 - increase distance to nearest HC indel call to 35, and nearest HC SNP call to 5
use strict;
use warnings;
...
my $qualHapprev=0;
my $chromprev2=1; #2nd previous chromosome name
my $posHapprev2=-1; #2nd previous Hap position
my $posUGprev2=-1; #2nd previous UG position
my $qualHapprev2=0;
...
while($endlineUG==1 || $endlineHap==1) {
    ...
    #if ($chromposUG>=62363174) {
   # 	print "$chromposUG,$chromposHap,$posHapprev,$posHapprev2,$qualHapprev,$qualHapprev2\n";
   # 	exit;
    #}
    ...
    if ($chromposUG==$chromposHap && $altUG eq $altHap && $gtUG eq $gtHap) { #same variant and genotype in both vcfs, so print UG
        ...
        $UGread=0;
        $Hapread=1;
    } elsif (($endlineHap==0 || $chromposUG<$chromposHap) && ($gtUG eq "0/0" || $gtUG eq "./.\n") && ($posHap-$posUG>20 || $qualHap<=40 || $varTypeHap eq "SNP") && ($posUG-$posHapprev>20 || $qualHapprev<=40 || $varTypeHapprev eq "SNP") && ($posUG-$posHapprev2>20 || $qualHapprev2<=40 || $varTypeHapprev2 eq "SNP")) { #no variant in Hap at this position in UG and UG genotype is HomRef or unknown and the 2 closest variants called by the HaplotypeCaller >20 (or the variant quality <=40 or it's a SNP), so print UG and increment UG
        ...
        $UGread=1;
        $Hapread=0;
#    } elsif (($endlineUG==0 || $chromposUG<$chromposHap) && ($posHap-$posUG>35 || $qualHap<=40) && ($posUG-$posHapprev>35 || $qualHapprev<=40) && $varTypeUG eq "SNP" ) { #no variant in Hap at this position in UG, UG genotype is not HomRef and the distance between this location and the closest variant called by the HaplotypeCaller >35 (or the variant quality <=40) and variant is a SNP, so print UG and increment UG
#    } elsif (($endlineUG==0 || $chromposUG<$chromposHap) && ($posHap-$posUG>35 || $qualHap<=40) && ($posUG-$posHapprev>35 || $qualHapprev<=40) ) { #no variant in Hap at this position in UG, UG genotype is not HomRef and the distance between this location and the closest variant called by the HaplotypeCaller >35 (or the variant quality <=40), so print UG and increment UG
#    } elsif (($endlineHap==0 || $chromposUG<$chromposHap) && ($posHap-$posUG>35 || $qualHap<=40 || $varTypeHap eq "SNP") && ($posUG-$posHapprev>35 || $qualHapprev<=40 || $varTypeHapprev eq "SNP")  && ($posUG-$posHapprev2>35 || $qualHapprev2<=40 || $varTypeHapprev2 eq "SNP")) { #no variant in Hap at this position in UG, UG genotype is not HomRef and the distance between this location and the closest variant called by the HaplotypeCaller >35 (or the variant quality <=40 or it's a SNP), so print UG and increment UG
    } elsif (($endlineHap==0 || $chromposUG<$chromposHap) && ($posHap-$posUG>35 || $qualHap<=40 || ($varTypeHap eq "SNP" && $posHap-$posUG>5)) && ($posUG-$posHapprev>35 || $qualHapprev<=40 || ($varTypeHapprev eq "SNP" && $posUG-$posHapprev>5)) && ($posUG-$posHapprev2>35 || $qualHapprev2<=40 || ($varTypeHapprev2 eq "SNP" && $posUG-$posHapprev>5))) { #no variant in Hap at this position in UG, UG genotype is not HomRef and the distance between this location and the closest variant called by the HaplotypeCaller >35 (or the variant quality <=40 or it's a SNP and >5bp away), so print UG and increment UG
		print OUTPUT "$chromUG\t$posUG\t$idUG\t$refUG\t$altUG\t$qualUG\t$filterUG\t$infoUG;UGHap=UG\t$formatUG\t$charUG";
        $UGread=1;
        $Hapread=0;
    } else { ##no variant in Hap at this position in UG, UG genotype is not HomRef and the distance between this location and the closest variant called by the HaplotypeCaller >35 or the Haplotype variant is a SNP, so print filtered UG and increment UG
...
sub readHap {
    $chromprev2=$chromprev2;
    $posHapprev2=$posHapprev;
    $qualHapprev2=$qualHapprev;
    $varTypeHapprev2=$varTypeHapprev;
    $chromprev=$chromHap;
    $posHapprev=$posHap;
    $qualHapprev=$qualHap;
